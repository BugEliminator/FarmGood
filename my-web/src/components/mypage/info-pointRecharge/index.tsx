...
export default function PointRecharge() {
  ...
  const onClickPointCharge = async () => {
    try {
      ...
      const result = await PortOne.requestPayment({
        ...
        orderName: "포인트 충전",
        totalAmount: Number(pointCharge), // 충전금액
  ...
  const onChangePoint = (event) => {
    setPointCharge(event.target.value);
  };

  return (
    <main className={styles.main}>
      <div className={styles.pointchargeSection}>
        <input
          onChange={onChangePoint}
          className={styles.pointInput}
          type="number"
          placeholder="원하시는 금액을 작성해 주세요."
        />

        <DollarOutlined className={styles.dollarIcon} />

        <button
          className={styles.pointchargeButton}
          onClick={onClickPointCharge}
        >
          포인트 충전하기
        </button>
      </div>
